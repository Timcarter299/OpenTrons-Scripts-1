...
def run(protocol: protocol_api.ProtocolContext):
    ...
    # lists
    detergent_vol = [79.3, 92.5, 123.33, 154.2] # from excel spreadsheet, trying a larger range
...
# Add lurb
    ...
    for col in cols[:3]:
        ...
        for row in rows[:7]:
            ...
            p300.touch_tip()
            p300.dispense(lurb_vol[count], plate[dest].bottom())
            sample_count += 1
        p300.drop_tip()
    count += 1
    for col in cols[3:6]:
    ...
    #     for row in rows:
    #         dest = row + str(col)
    #         p300.aspirate(detergent_vol[count], detergent[detergent_count].bottom())
    #         p300.touch_tip()
    #         p300.dispense(detergent_vol[count], plate[dest])
    #     count += 1
    # detergent_count += 1
    # p300.drop_tip()
    ...
    # # add water to last row
    # count = 0
    # for col in cols[:3]:
    #     p300.pick_up_tip()
    #     for row in rows[7:]:
    #         dest = row + str(col)
    #         p300.aspirate(lurb_vol[count], lurbnl)
    #         p300.touch_tip()
    #         p300.dispense(lurb_vol[count], plate[dest].bottom())
    #     p300.drop_tip()
    # count += 1
    ...
    #         p300.aspirate(lurb_vol[count], lurbnl)
    #         p300.touch_tip()
    #         p300.dispense(lurb_vol[count], plate[dest].bottom(5))
